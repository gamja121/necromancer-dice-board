...
const padded = new Uint8ClampedArray(10 * 10 * 4);
for (const x of [2, 6]) padded[(7 * 10 + x) * 4 + 3] = 255;
assert.deepEqual(effect.footAnchor(padded, 10, 10), { x: .45, y: .8 });
assert.deepEqual(effect.footAnchor(new Uint8ClampedArray(400), 10, 10), { x: .5, y: 1 });
global.document = { createElement: () => ({ getContext: () => ({ drawImage() {}, getImageData: () => ({ data: padded }) }) }) };
const circle = { style: {} };
effect.placeAtFeet(circle, { naturalWidth: 10, naturalHeight: 10, clientWidth: 200, clientHeight: 100, offsetLeft: -20, offsetTop: 5 });
assert.equal(circle.style.left, "75px");
