...
public static class MimicSheetProcessor
{
    ...
    private static void RemoveFinalSparkle(Bitmap bitmap, Rectangle cell)
    {
        ...
        for (int x = 0; x < bitmap.Width; x++)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (globalX >= 1135 && globalX <= 1225 && globalY >= 430 && globalY <= 520)
    ...
    {
        const int width = 320, height = 250;
    ...
    public static void Process(string inputPath, string outputDirectory)
    {
        ...
        Rectangle[][] cells = {
            new Rectangle[] {
                Rectangle.FromLTRB(108, 0, 312, 200), Rectangle.FromLTRB(316, 0, 530, 200),
                Rectangle.FromLTRB(532, 0, 784, 200), Rectangle.FromLTRB(786, 0, 1074, 200),
                Rectangle.FromLTRB(1072, 0, 1278, 200)
            },
            new Rectangle[] {
                Rectangle.FromLTRB(125, 204, 334, 400), Rectangle.FromLTRB(338, 204, 540, 400),
                Rectangle.FromLTRB(544, 204, 750, 400), Rectangle.FromLTRB(760, 204, 966, 400)
            },
            new Rectangle[] {
                Rectangle.FromLTRB(118, 402, 324, 574), Rectangle.FromLTRB(316, 402, 512, 574),
                Rectangle.FromLTRB(504, 402, 700, 574), Rectangle.FromLTRB(692, 402, 892, 574),
                Rectangle.FromLTRB(900, 402, 1082, 574), Rectangle.FromLTRB(1090, 402, 1278, 574)
            }
        };
...
[MimicSheetProcessor]::Process((Resolve-Path $InputPath), $OutputDirectory)
Write-Output "Processed mimic sheet into $OutputDirectory"
