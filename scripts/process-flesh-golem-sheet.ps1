...
public static class FleshGolemSheetProcessor
{
    private static Color RemoveMagenta(Color c)
    {
        ...
        if (excess > 8)
        {
            // Soft matte preserves fine outlines contaminated by JPEG magenta.
    ...
    public static void Process(string input, string output, string previewPath)
    {
        ...
        // Explicit source bounds: attack 1-5, hit 1-5, death 1-6.
        int[][] edges = { new [] {112, 322, 562, 798, 1037, 1268},
                          new [] {112, 322, 562, 798, 1037, 1268},
                          new [] {134, 303, 499, 689, 871, 1064, 1264} };
        int[] tops = {0, 198, 402}, bottoms = {198, 402, 575};
        ...
        {
            if (source.Width != 1280 || source.Height != 575)
                throw new InvalidOperationException("Expected the original 1280x575 FleshGolem sheet.");
            ...
            for (int index = 0; index < edges[row].Length - 1; index++)
            {
                ...
                using (var frame = new Bitmap(width, height, PixelFormat.Format32bppArgb))
                {
                    ...
                    // The supplied death row reaches the JPEG's last scanline.
                    // Preserve it in full; only internal crop edges indicate a bad cut.
                    if (left == 0 || right == cellWidth - 1 || top == 0 ||
                        (bottom == cellHeight - 1 && bottoms[row] < source.Height))
...
[FleshGolemSheetProcessor]::Process((Resolve-Path $InputPath), $OutputDirectory, $PreviewPath)
