...
assert(html.includes("unit-info-window-no-portrait.png?v=1"), "Unit information window cache version is stale.");
...
assert(html.indexOf('src="unit-data.js?v=55"') < html.indexOf('src="v2-auto-battle-practice.js'), "Shared unit metadata must load before the battle page.");
...
assert(source.includes('unitInfoImage.setAttribute("href", unitState.infoPortrait || unitState.portrait)'), "Unit information must prefer its dedicated portrait and retain the original cutout fallback.");
...
assert(worker.includes('necromancer-expedition-v234'), "Service worker cache version was not advanced.");
...
assert(worker.includes("v2-auto-battle-practice.js?v=63") && worker.includes("v2-legions.js?v=3") && worker.includes("v2-battle-brands.js?v=3"), "Turn-based status battle logic is not cached.");
...
for (const slug of ["abyss-claw-hunter", "corpse-slime", "medusa", "siren", "abyss-harpy", "grave-worm", "goblin-commoner", "spiderling", "skeleton-cavalry", "mummy-guardian", "soul-reaper", "bone-hound", "mimic", "ice-princess", "hydra", "flesh-golem", "forest-fairy", "bone-golem", "boulder-ogre", "orc-warrior"]) {
...
assert(fs.existsSync(path.join(root, "art/v2-style/ui/unit-info-window-no-portrait.png")) && worker.includes("art/v2-style/ui/unit-info-window-no-portrait.png?v=1"), "Portrait-box-free unit info frame must exist and be cached.");
...
for (const [slug, grade, legions] of [["death-knight", "hero", ["demon"]], ["skeleton-spear", "normal", ["skeleton"]], ["ghoul", "normal", ["corpse"]], ["ancient-treant", "advanced", ["plant", "element"]], ["goblin-rider", "normal", ["beast"]], ["orc-warrior", "advanced", ["beast"]], ["boulder-ogre", "advanced", ["beast"]], ["minotaur", "advanced", ["beast"]], ["abyss-claw-hunter", "advanced", ["insect", "plague"]], ["corpse-slime", "normal", ["corpse"]]]) {
...
assert(vm.runInContext("ROSTER.length", infoContext) === 45, "Every attack-capable unit must appear in the ally picker.");
...
for (const slug of ["abyss-claw-hunter", "corpse-slime", "siren", "abyss-harpy", "grave-worm", "goblin-commoner", "spiderling", "skeleton-cavalry", "mummy-guardian", "soul-reaper", "bone-hound", "mimic", "ice-princess", "hydra", "flesh-golem", "forest-fairy", "bone-golem", "boulder-ogre", "orc-warrior"]) {
...
assert(infoContext.unitInfoPortrait.attrs.viewBox === "0 0 192 288" && infoContext.unitInfoImage.attrs.x === "0" && infoContext.unitInfoImage.attrs.y === "0" && infoContext.unitInfoImage.attrs.width === "192" && infoContext.unitInfoImage.attrs.height === "288", "The complete framed portrait must overlay the blank information-window area without cropping.");
vm.runInContext("closeUnitInfo()", infoContext);
...
vm.runInContext("openUnitInfo(selected)", infoContext);
assert(infoContext.unitInfoImage.attrs.href === orc.infoPortrait && orc.infoPortrait === "art/v2-style/ui/info-portraits/orc-warrior.png?v=1" && infoContext.unitInfoName.textContent === "오크 전사", "Actual Orc info window must display the dedicated portrait.");
