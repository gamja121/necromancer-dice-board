...
assert(html.indexOf('src="unit-data.js?v=54"') < html.indexOf('src="v2-auto-battle-practice.js'), "Shared unit metadata must load before the battle page.");
...
assert(worker.includes('necromancer-expedition-v220'), "Service worker cache version was not advanced.");
...
assert(worker.includes("v2-unit-cards.js?v=17"), "Summoned-unit card mapping is not cached.");
for (const slug of ["goblin-commoner","sea-wolf","grave-priest","abyss-eye","doom-executor","death-knight","hell-mantis","scorpion-knight","ancient-treant","stone-golem"]) assert(worker.includes(`art/v2-style/ui/unit-card-${slug}.png?v=13`), `Replacement ${slug} card art is not cached.`);
