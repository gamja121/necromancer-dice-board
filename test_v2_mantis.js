...
const api = require("./v2-mantis-frames.js");
...
const document = { createElement() {
  ...
  const canvas = { width: 0, height: 0, getContext() { return {
    ...
    assert.equal(canvas.width, 280); assert.equal(canvas.height, 260);
    const [sx, sy, sw, sh, dx, dy, dw, dh] = drawn;
    assert(sw > 40 && sh > 40);
    assert(dx >= 8 && dy >= 8 && dx + dw <= 272 && dy + dh === 250, "Safe edges and a shared foot baseline");
...
assert.equal(result.attack.length, 5); assert.equal(result.hit.length, 4); assert.equal(result.death.length, 6);
assert.equal(result.idle, result.attack[0]);
assert.deepEqual(frames.flatMap((f,i) => f.flip ? [i] : []), [4], "Only attack frame 5 is mirrored");
...
assert(html.includes('data-unit="hell-mantis"'));
...
console.log("PASS: 15 actual JPEG crops retain edge margins; only attack 5 mirrored; transparency, shared baseline, picker and loader verified.");
console.log(frames);
async function testPlayer() {
  ...
  const context = {
    console, URLSearchParams, window: { location: { search: "?unit=hell-mantis" } },
  ...
  assert.equal(elements.get("#unitName").textContent, "지옥 사마귀");
