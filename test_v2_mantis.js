...
const document = { createElement() {
  ...
  const canvas = { width: 0, height: 0, getContext() { return {
    ...
    assert.equal(canvas.width, canvasWidth); assert.equal(canvas.height, canvasHeight);
    const [sx, sy, sw, sh, dx, dy, dw, dh] = drawn;
    assert(sw > 40 && sh > 40);
    assert(dx >= 8 && dy >= 8 && dx + dw <= canvasWidth - 8 && dy + dh === canvasHeight - 10, "Safe edges and a shared foot baseline");
