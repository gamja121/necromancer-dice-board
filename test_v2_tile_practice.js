...
const inheritance = { hidden: true, focus() { handlers.focused = true; }, addEventListener: (name, fn) => { handlers.inheritance = fn; } };
const exit = { addEventListener: (name, fn) => { handlers.exit = fn; } };
const navigation = [];
vm.runInNewContext(script, {
  document: { getElementById: (id) => ({ tileTestImage: image, tileTestEnter: enter, tileTestInheritance: inheritance, tileTestExit: exit })[id] },
  V2HomeInheritance: { open: () => { handlers.boardOpened = true; } },
  window: { location: { assign: (url) => navigation.push(url) } }
});
handlers.enter();
if (!image.src.endsWith(art) || !enter.hidden || inheritance.hidden || !handlers.focused) throw Error("Enter must swap to interior and show inheritance action");
handlers.inheritance();
if (!handlers.boardOpened) throw Error("Inheritance action must open the two-panel board");
handlers.exit();
